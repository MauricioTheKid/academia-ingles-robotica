...
public class Calificacion {
    ...
    public BigDecimal getPromedio() {
        ...
        if (notaParcial1 != null) {
            sum = sum.add(notaParcial1);
            count++;
        }
        if (notaParcial2 != null) {
            sum = sum.add(notaParcial2);
            count++;
        }
        if (notaParcial3 != null) {
            sum = sum.add(notaParcial3);
            count++;
        }
        if (notaParcial4 != null) {
            sum = sum.add(notaParcial4);
            count++;
        }
        if (notaProyecto != null) {
            sum = sum.add(notaProyecto);
            count++;
        }
        if (notaExamenFinal != null) {
            sum = sum.add(notaExamenFinal);
            count++;
        }
    ...
    public String getEstado() {
        ...
        if (promedio.compareTo(BigDecimal.ZERO) == 0)
            return "Sin notas";
        if (promedio.compareTo(new BigDecimal("7.0")) >= 0)
            return "Aprobado";
        if (promedio.compareTo(new BigDecimal("5.0")) >= 0)
            return "Regular";
        return "Reprobado";
    }
